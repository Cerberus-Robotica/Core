...
namespace roles {
    void RoleKickOffKicker::act(RobotController& robot) {
		...
		std::cout << others_positioned << robot.mWorld.isAllAlliesOnOurSideorOnCenterCircle() << (robot.mTeam->event == TeamInfo::ourKickOff) << (robot.mWorld.ball.getPosition().getDistanceTo({0, 0}) < 200) << std::endl;
