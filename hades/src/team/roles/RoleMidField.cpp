    ...
    void RoleMidField::act(RobotController& robot) {
        ...
        if (robot.mWorld.ball.getVelocity().getNorm() != 0 && interceptor < 20 && robot.mTeam->num_of_active_robots > 1) {
            ...
            if (closest_allies_to_ball.size() > 0) {
                ...
                Pivot_id = robot.getId();
            }

        }


        if (Pivot_id == robot.getId() || Pivot_id == -1) {
            if (robot.mWorld.ball.getVelocity().getNorm() != 0) {
                keepLocation.act(robot, robot.getPosition());
            }
            ...
            double mid_field_dislocation = fabs(robot.mWorld.ball.getPosition().getX());
            ...
            double delta_y = sqrt(pow(robot.mKick_distance, 2) - pow(x_position - robot.mWorld.ball.getPosition().getX(), 2));
            double y_position;
            ...
            positionAndKick.act(robot, wait_position);
